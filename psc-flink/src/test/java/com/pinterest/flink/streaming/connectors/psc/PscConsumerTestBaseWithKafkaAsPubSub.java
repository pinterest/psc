...
import com.pinterest.flink.streaming.connectors.psc.testutils.ClusterCommunicationUtils;
import com.pinterest.flink.streaming.connectors.psc.testutils.DataGenerators;
import com.pinterest.flink.streaming.connectors.psc.testutils.FailingIdentityMapper;
import com.pinterest.flink.streaming.connectors.psc.testutils.PartitionValidatingMapper;
...
public abstract class PscConsumerTestBaseWithKafkaAsPubSub extends PscTestBaseWithFlinkWithKafkaAsPubSub {
    ...
    /**
     * Makes sure that no job is on the JobManager any more from any previous tests
     * that use the same mini cluster. Otherwise, missing slots may happen.
     */
    @Before
    ...
    /**
     * Test that ensures the KafkaConsumer is properly failing if the topic doesnt
     * exist and a wrong broker was specified.
     *
     * @throws Exception
     */
    public void runFailOnNoBrokerTest() throws Exception {
        try {
            ...
            pscConsumerConfiguration.setProperty(PscConfiguration.PSC_CONSUMER_GROUP_ID, "test");
            pscConsumerConfiguration.setProperty("psc.consumer.default.api.timeout.ms", "3000"); // let the test fail fast
            pscConsumerConfiguration.setProperty("psc.consumer.socket.timeout.ms", "3000");
            pscConsumerConfiguration.setProperty("psc.consumer.session.timeout.ms", "2000");
            ...
            if (pscTestEnvWithKafka.getVersion().equals("0.9") || pscTestEnvWithKafka.getVersion().equals("0.10")
    ...
    public void runCommitOffsetsToKafka() throws Exception {
        ...
        final TopicUri topicUri = writeSequence(PscTestEnvironmentWithKafkaAsPubSub.PSC_TEST_TOPIC_URI_PREFIX +
        ...
        PscTestEnvironmentWithKafkaAsPubSub.PscOffsetHandler pscOffsetHandler = pscTestEnvWithKafka.createOffsetHandler(topicUri.getTopicUriAsString());
    ...
    /**
     * This test ensures that when the consumers retrieve some start offset from
     * kafka (earliest, latest), that this offset is committed to Kafka, even if
     * some partitions are not read.
     *
     * <p>
     * Test: - Create 3 partitions - write 50 messages into each. - Start three
     * consumers with auto.offset.reset='latest' and wait until they committed into
     * Kafka. - Check if the offsets in Kafka are set to 50 for the three partitions
     *
     * <p>
    ...
    public void runAutoOffsetRetrievalAndCommitToKafka() throws Exception {
        ...
        Properties readConfiguration = new Properties();
        readConfiguration.putAll(standardPscConsumerConfiguration);
    ...
    /**
     * This test ensures that when explicitly set to start from earliest record, the
     * consumer ignores the PscConfiguration.PSC_CONSUMER_OFFSET_AUTO_RESET behaviour as well as any committed
    ...
    public void runStartFromEarliestOffsets() throws Exception {
        ...
        PscTestEnvironmentWithKafkaAsPubSub.PscOffsetHandler pscOffsetHandler = pscTestEnvWithKafka.createOffsetHandler(topicUri.getTopicUriAsString());
        pscOffsetHandler.setCommittedOffset(topicUri.getTopicUriAsString(), 0, 23);
        pscOffsetHandler.setCommittedOffset(topicUri.getTopicUriAsString(), 1, 31);
        pscOffsetHandler.setCommittedOffset(topicUri.getTopicUriAsString(), 2, 43);

        readSequence(env, StartupMode.EARLIEST, null, null, readConfiguration, parallelism, topicUri.getTopicUriAsString(),
                     recordsInEachPartition, 0);
    ...
    /**
     * This test ensures that when explicitly set to start from latest record, the
     * consumer ignores the PscConfiguration.PSC_CONSUMER_OFFSET_AUTO_RESET behaviour as well as any committed
     * group offsets in Kafka.
     */
    public void runStartFromLatestOffsets() throws Exception {
        // 50 records written to each of 3 partitions before launching a latest-starting
        // consuming job
        final int parallelism = 3;
        final int recordsInEachPartition = 50;
        ...
        // serialization / deserialization schemas for writing and consuming the extra
        // records
        final TypeInformation<Tuple2<Integer, Integer>> resultType = TypeInformation
                .of(new TypeHint<Tuple2<Integer, Integer>>() {
                });

        final SerializationSchema<Tuple2<Integer, Integer>> serSchema = new TypeInformationSerializationSchema<>(
                resultType, new ExecutionConfig());

        final PscDeserializationSchema<Tuple2<Integer, Integer>> deserSchema = new PscDeserializationSchemaWrapper<>(
        ...
        final Properties readConfiguration = new Properties();
        readConfiguration.putAll(standardPscConsumerConfiguration);
        readConfiguration.putAll(pscDiscoveryConfiguration);
        readConfiguration.setProperty(
                PscConfiguration.PSC_CONSUMER_OFFSET_AUTO_RESET,
        ...
        DataStream<Tuple2<Integer, Integer>> extraRecordsStream = env2
                .addSource(new RichParallelSourceFunction<Tuple2<Integer, Integer>>() {
                    ...
                    public void run(SourceContext<Tuple2<Integer, Integer>> ctx) throws Exception {
                        int count = recordsInEachPartition; // the extra records should start from the last
                        // written value
        ...
        pscTestEnvWithKafka.produceIntoKafka(extraRecordsStream, topicUri.getTopicUriAsString(), serSchema, pscProducerProperties, null);
        ...
        // test will fail here if the consume job had incorrectly read any records other
        // than the extra records
    ...
    /**
     * This test ensures that the consumer correctly uses group offsets in Kafka,
     * and defaults to PscConfiguration.PSC_CONSUMER_OFFSET_AUTO_RESET behaviour when necessary, when explicitly
     * configured to start from group offsets.
     *
     * <p>
     * The partitions and their committed group offsets are setup as: partition 0
     * --> committed offset 23 partition 1 --> no commit offset partition 2 -->
     * committed offset 43
     *
     * <p>
     ...
     * partition 1 --> default to PscConfiguration.PSC_CONSUMER_OFFSET_AUTO_RESET (set to earliest), so start
     * from offset 0, read to offset 49 (50 records) partition 2 --> start from
     * offset 43, read to offset 49 (7 records)
     */
    public void runStartFromGroupOffsets() throws Exception {
        ...
        partitionsToValueCountAndStartOffsets.put(0, new Tuple2<>(27, 23)); // partition 0 should read
        // offset 23-49
        partitionsToValueCountAndStartOffsets.put(1, new Tuple2<>(50, 0)); // partition 1 should read
        // offset 0-49
        partitionsToValueCountAndStartOffsets.put(2, new Tuple2<>(7, 43)); // partition 2 should read
        // offset 43-49

        readSequence(env, StartupMode.GROUP_OFFSETS, null, null, readConfiguration, topicUri.getTopicUriAsString(),
    ...
    /**
     * This test ensures that the consumer correctly uses user-supplied specific
     * offsets when explicitly configured to start from specific offsets. For
     * partitions which a specific offset can not be found for, the starting
     * position for them should fallback to the group offsets behaviour.
     *
     * <p>
     * 4 partitions will have 50 records with offsets 0 to 49. The supplied specific
     * offsets map is: partition 0 --> start from offset 19 partition 1 --> not set
     * partition 2 --> start from offset 22 partition 3 --> not set partition 4 -->
     * start from offset 26 (this should be ignored because the partition does not
     * exist)
     *
     * <p>
     * The partitions and their committed group offsets are setup as: partition 0
     * --> committed offset 23 partition 1 --> committed offset 31 partition 2 -->
     * committed offset 43 partition 3 --> no commit offset
     *
     * <p>
     * When configured to start from these specific offsets, each partition should
     * read: partition 0 --> start from offset 19, read to offset 49 (31 records)
     * partition 1 --> fallback to group offsets, so start from offset 31, read to
     * offset 49 (19 records) partition 2 --> start from offset 22, read to offset
     * 49 (28 records) partition 3 --> fallback to group offsets, but since there is
     * no group offset for this partition, will default to PscConfiguration.PSC_CONSUMER_OFFSET_AUTO_RESET (set
     * to "earliest"), so start from offset 0, read to offset 49 (50 records)
     */
    public void runStartFromSpecificOffsets() throws Exception {
        ...
        Map<PscTopicUriPartition, Long> specificStartupOffsets = new HashMap<>();
        specificStartupOffsets.put(new PscTopicUriPartition(topicUri.getTopicUriAsString(), 0), 19L);
        specificStartupOffsets.put(new PscTopicUriPartition(topicUri.getTopicUriAsString(), 2), 22L);
        specificStartupOffsets.put(new PscTopicUriPartition(topicUri.getTopicUriAsString(), 4), 26L); // non-existing partition, should be ignored

        // only the committed offset for partition 1 should be used, because partition 1
        // has no entry in specific offset map
        PscTestEnvironmentWithKafkaAsPubSub.PscOffsetHandler pscOffsetHandler = pscTestEnvWithKafka.createOffsetHandler(topicUri.getTopicUriAsString());
        pscOffsetHandler.setCommittedOffset(topicUri.getTopicUriAsString(), 0, 23);
        pscOffsetHandler.setCommittedOffset(topicUri.getTopicUriAsString(), 1, 31);
        pscOffsetHandler.setCommittedOffset(topicUri.getTopicUriAsString(), 2, 43);
        ...
        readSequence(env, StartupMode.SPECIFIC_OFFSETS, specificStartupOffsets, null, readConfiguration,
                topicUri.getTopicUriAsString(), partitionsToValueCountAndStartOffsets);
    ...
    /**
     * This test ensures that the consumer correctly uses user-supplied timestamp
     * when explicitly configured to start from timestamp.
     *
     * <p>
     * The validated Kafka data is written in 2 steps: first, an initial 50 records
     * is written to each partition. After that, another 30 records is appended to
     * each partition. Before each step, a timestamp is recorded. For the
     * validation, when the read job is configured to start from the first
     * timestamp, each partition should start from offset 0 and read a total of 80
     * records. When configured to start from the second timestamp, each partition
     * should start from offset 50 and read on the remaining 30 appended records.
     */
    public void runStartFromTimestamp() throws Exception {
        ...
        // attempt to create an appended test sequence, where the timestamp of writing
        ...
        writeAppendSequence(topicUri.getTopicUriAsString(), initialRecordsInEachPartition, appendRecordsInEachPartition,
                parallelism);
        ...
        readConfiguration.putAll(standardPscConsumerConfiguration);
        readConfiguration.putAll(pscDiscoveryConfiguration);
        readSequence(env, StartupMode.TIMESTAMP, null, firstTimestamp, readConfiguration, parallelism, topicUri.getTopicUriAsString(),
                initialRecordsInEachPartition + appendRecordsInEachPartition, 0);
        readSequence(env, StartupMode.TIMESTAMP, null, secondTimestamp, readConfiguration, parallelism, topicUri.getTopicUriAsString(),
                appendRecordsInEachPartition, initialRecordsInEachPartition);

        deleteTestTopic(topicUri.getTopic());
    }

    /**
     * Ensure Kafka is working on both producer and consumer side. This executes a
     * job that contains two Flink pipelines.
     *
     * <pre>
     ...
     *
     * <p>
     * We need to externally retry this test. We cannot let Flink's retry mechanism
     * do it, because the Kafka producer does not guarantee exactly-once output.
     * Hence a recovery would introduce duplicates that cause the test to fail.
     *
     * <p>
    ...
    public void runSimpleConcurrentProducerConsumerTopology() throws Exception {
        ...
        final String additionalEmptyTopicUri = PscTestEnvironmentWithKafkaAsPubSub.PSC_TEST_TOPIC_URI_PREFIX + additionalEmptyTopic;
        ...
        TypeInformation<Tuple2<Long, String>> longStringType = TypeInformation
                .of(new TypeHint<Tuple2<Long, String>>() {
                });
        ...
        pscTestEnvWithKafka.produceIntoKafka(stream, topicUri, sinkSchema, producerProperties, null);

        // ----------- add consumer dataflow ----------

        List<String> topicUris = new ArrayList<>();
        topicUris.add(topicUri);
        topicUris.add(additionalEmptyTopicUri);
        ...
        deleteTestTopic(additionalEmptyTopic);
    }

    /**
     * Tests the proper consumption when having a 1:1 correspondence between kafka
     * partitions and Flink sources.
     */
    ...
    /**
     * Tests the proper consumption when having fewer Flink sources than Kafka
     * partitions, so one Flink source will read multiple Kafka partitions.
     */
    public void runOneSourceMultiplePartitionsExactlyOnceTest() throws Exception {
        ...
        DataGenerators.generateRandomizedIntegerSequence(
                StreamExecutionEnvironment.getExecutionEnvironment(), pscTestEnvWithKafka, topicUri, numPartitions,
                numElementsPerPartition, true);
    ...
    /**
     * Tests the proper consumption when having more Flink sources than Kafka
     * partitions, which means that some Flink sources will read no partitions.
     */
    public void runMultipleSourcesOnePartitionExactlyOnceTest() throws Exception {
        ...
        env.enableCheckpointing(500);
        env.setParallelism(parallelism);
        // set the number of restarts to one. The failing mapper will fail once, then
        // it's only success exceptions.
        env.setRestartStrategy(RestartStrategies.fixedDelayRestart(1, 0));
    ...
    public void runCancelingOnFullInputTest() throws Exception {
        ...
        DataGenerators.InfiniteStringsGenerator generator = new DataGenerators.InfiniteStringsGenerator(
                pscTestEnvWithKafka, topicUri);
        generator.start();
        ...
        // wait for the program to be done and validate that we failed with the right
        // exception
        runnerThread.join();
    ...
    public void runProduceConsumeMultipleTopics(boolean useLegacySchema) throws Exception {
                    ...
                    public void run(SourceContext<Tuple3<Integer, Integer, String>> ctx) throws Exception {
                        ...
                        for (int topicId = 0; topicId < numTopics; topicId++) {
                            for (int i = 0; i < numElements; i++) {
                                ctx.collect(new Tuple3<>(partition, i, PscTestEnvironmentWithKafkaAsPubSub.PSC_TEST_TOPIC_URI_PREFIX + "topic-" + topicId));
                            }
                        }
                    }
        ...
        pscProducerConfiguration.putAll(standardPscProducerConfiguration);
        pscProducerConfiguration.putAll(securePscProducerConfiguration);
        pscProducerConfiguration.putAll(pscDiscoveryConfiguration);

        if (useLegacySchema) {
            Tuple2WithTopicSchema schema = new Tuple2WithTopicSchema(env.getConfig());
            pscTestEnvWithKafka.produceIntoKafka(
                    stream,
                    PscTestEnvironmentWithKafkaAsPubSub.PSC_TEST_TOPIC_URI_PREFIX + "test",
                    schema,
        ...
        pscConsumerProperties.putAll(securePscConsumerConfiguration);
        ...
        stream.flatMap(new FlatMapFunction<Tuple3<Integer, Integer, String>, Integer>() {
            ...
            @Override
            public void flatMap(Tuple3<Integer, Integer, String> value,
    ...
    public void runBigRecordTestTopology() throws Exception {
        ...
        final TypeInformation<Tuple2<Long, byte[]>> longBytesInfo = TypeInformation
                .of(new TypeHint<Tuple2<Long, byte[]>>() {
                });
        ...
        Properties pscConsumerConfiguration = new Properties();
        pscConsumerConfiguration.putAll(standardPscConsumerConfiguration);
        pscConsumerConfiguration.setProperty("psc.consumer.fetch.message.max.bytes", Integer.toString(1024 * 1024 * 14));
        pscConsumerConfiguration.setProperty(
                PscConfiguration.PSC_CONSUMER_PARTITION_FETCH_MAX_BYTES,
                Integer.toString(1024 * 1024 * 14)
        ); // for the new fetcher
        pscConsumerConfiguration.setProperty("psc.consumer.queued.max.message.chunks", "1");
        pscConsumerConfiguration.putAll(securePscConsumerConfiguration);
        pscConsumerConfiguration.putAll(pscDiscoveryConfiguration);
        ...
        pscProducerConfiguration.putAll(standardPscProducerConfiguration);
        pscProducerConfiguration.putAll(securePscProducerConfiguration);
        pscProducerConfiguration.setProperty(PscConfiguration.PSC_PRODUCER_REQUEST_SIZE_MAX_BYTES, Integer.toString(1024 * 1024 * 15));
        pscProducerConfiguration.setProperty(PscConfiguration.PSC_PRODUCER_RETRIES, "3");
        pscProducerConfiguration.putAll(pscDiscoveryConfiguration);

        DataStream<Tuple2<Long, byte[]>> stream = env
                .addSource(new RichSourceFunction<Tuple2<Long, byte[]>>() {
        ...
        pscTestEnvWithKafka.produceIntoKafka(stream, topicUri, serSchema, pscProducerConfiguration, null);
    ...
    public void runKeyValueTest() throws Exception {
        ...
        DataStream<Tuple2<Long, PojoValue>> kvStream = env
                .addSource(new SourceFunction<Tuple2<Long, PojoValue>>() {
                    @Override
        ...
        Properties producerConfiguration = standardPscProducerConfiguration;
        producerConfiguration.putAll(pscDiscoveryConfiguration);
        producerConfiguration.setProperty("psc.producer.retries", "3");
        pscTestEnvWithKafka.produceIntoKafka(kvStream, topicUri, schema, producerConfiguration, null);
        ...
        PscDeserializationSchema<Tuple2<Long, PojoValue>> readSchema = new TypeInformationKeyValueSerializationSchema<>(
                Long.class, PojoValue.class, env.getConfig());
    ...
    public void runAllDeletesTest() throws Exception {
        ...
        producerProperties.putAll(standardPscProducerConfiguration);
        producerProperties.putAll(pscDiscoveryConfiguration);
        pscTestEnvWithKafka.produceIntoKafka(kvStream, topicUri, schema, producerProperties, null);
    ...
    /**
     * Test that ensures that DeserializationSchema.isEndOfStream() is properly
     * evaluated.
     *
     * @throws Exception
     */
    ...
    /**
     * Test that ensures that DeserializationSchema can emit multiple records via a
     * Collector.
     *
     * @throws Exception
     */
    public void runCollectingSchemaTest() throws Exception {
        ...
        DataStream<Tuple2<Integer, String>> fromKafka = env1.addSource(pscTestEnvWithKafka
                .getPscConsumer(topicUri.getTopicUriAsString(), new CollectingDeserializationSchema(elementCount), pscConsumerConfiguration)
    ...
    public void runMetricsTest() throws Throwable {
        ...
        pscTestEnvWithKafka.produceIntoKafka(fromGen, topicUri, schema, standardPscConsumerConfiguration, null);
        ...
        try {
            ...
            Set<ObjectName> offsetMetrics = mBeanServer.queryNames(new ObjectName("*current-offsets*:*"),
                    null);
    ...
    private static class CollectingDeserializationSchema
            implements PscDeserializationSchema<Tuple2<Integer, String>> {

        final int finalCount;

        TypeInformation<Tuple2<Integer, String>> ti = TypeInformation
                .of(new TypeHint<Tuple2<Integer, String>>() {
                });
        TypeSerializer<Tuple2<Integer, Integer>> ser = TypeInformation
                .of(new TypeHint<Tuple2<Integer, Integer>>() {
        ...
        @Override
        public Tuple2<Integer, String> deserialize(PscConsumerMessage<byte[], byte[]> record) throws Exception {
            throw new UnsupportedOperationException("Should not be called");
        }

        @Override
        public void deserialize(PscConsumerMessage<byte[], byte[]> message,
                                Collector<Tuple2<Integer, String>> out) throws Exception {
            DataInputView in = new DataInputViewStreamWrapper(
                    new ByteArrayInputStream(message.getValue()));
    ...
    private static class FixedNumberDeserializationSchema
        ...
        TypeInformation<Tuple2<Integer, Integer>> ti = TypeInformation
                .of(new TypeHint<Tuple2<Integer, Integer>>() {
                });
    ...
    /**
     * Runs a job using the provided environment to read a sequence of records from
     * a single Kafka topic. The method allows to individually specify the expected
     * starting offset and total read value count of each partition. The job will be
     * considered successful only if all partition read results match the start
     * offset and value count criteria.
     */
    protected void readSequence(final StreamExecutionEnvironment env,
                                final StartupMode startupMode,
                                final Map<PscTopicUriPartition, Long> specificStartupOffsets,
        ...
        final TypeInformation<Tuple2<Integer, Integer>> intIntTupleType = TypeInformation
                .of(new TypeHint<Tuple2<Integer, Integer>>() {
                });
        ...
        // create the consumer
        //cc.putAll(standardPscConsumerConfiguration);
        cc.putAll(securePscConsumerConfiguration);
        cc.putAll(pscDiscoveryConfiguration);
...
//    cc.setProperty("psc.discovery.security.protocols", "plaintext");
    ...
    /**
     * Variant of
     * {@link PscConsumerTestBaseWithKafkaAsPubSub#readSequence(StreamExecutionEnvironment, StartupMode, Map, Long, Properties, String, Map)}
     * to expect reading from the same start offset and the same value count for all
     * partitions of a single Kafka topic.
     */
    protected void readSequence(final StreamExecutionEnvironment env,
                                final StartupMode startupMode,
                                final Map<PscTopicUriPartition, Long> specificStartupOffsets,
    ...
    protected void setPscConsumerOffset(final StartupMode startupMode,
                                        final FlinkPscConsumerBase<Tuple2<Integer, Integer>> consumer,
                                        final Map<PscTopicUriPartition, Long> specificStartupOffsets,
                                        final Long startupTimestamp) {
        switch (startupMode) {
            case EARLIEST:
    ...
    protected TopicUri writeSequence(String baseTopicUri,
                                     final int numElements,
                                     final int parallelism,
                                     final int replicationFactor) throws Exception {
        LOG.info("\n===================================\n" + "== Writing sequence of " + numElements
                + " into " + baseTopicUri + " with p=" + parallelism + "\n"
                + "===================================");
        ...
        for (int attempt = 1; attempt <= maxNumAttempts; attempt++) {
            ...
            final String topicUriStr = baseTopicUri + '-' + attempt;
            ...
            createTestTopic(topicUri.getTopic(), parallelism, replicationFactor);
            ...
            Properties pscProducerConfiguration = new Properties();
            pscProducerConfiguration.putAll(securePscProducerConfiguration);
            pscProducerConfiguration.setProperty(PscConfiguration.PSC_PRODUCER_CLIENT_ID, "flink-sequence-writer-producer");
            pscProducerConfiguration.setProperty(PscConfiguration.PSC_METRIC_REPORTING_ENABLED, "false");
            pscProducerConfiguration.setProperty("psc.producer.retries", "0");
            pscProducerConfiguration.putAll(pscDiscoveryConfiguration);

            pscTestEnvWithKafka.produceIntoKafka(stream, topicUriStr, serSchema, pscProducerConfiguration,
                    new Tuple2FlinkPartitioner(parallelism)).setParallelism(parallelism);
            ...
            if (validateSequence(topicUriStr, parallelism, deserSchema, numElements)) {
                // everything is good!
                return topicUri;
            } else {
                deleteTestTopic(topicUri.getTopic());
    ...
    protected void writeAppendSequence(String topicUri,
        ...
        pscProducerConfiguration.setProperty(PscConfiguration.PSC_PRODUCER_CLIENT_ID, "flink-sequence-writer-producer");
        pscProducerConfiguration.setProperty(PscConfiguration.PSC_METRIC_REPORTING_ENABLED, "false");
        pscProducerConfiguration.setProperty(PscConfiguration.PSC_PRODUCER_RETRIES, "0");
        pscProducerConfiguration.putAll(pscDiscoveryConfiguration);

        pscTestEnvWithKafka.produceIntoKafka(stream, topicUri, serSchema, pscProducerConfiguration,
                new Tuple2FlinkPartitioner(parallelism)).setParallelism(parallelism);
        ...
        if (!validateSequence(topicUri, parallelism, deserSchema,
                originalNumElements + numElementsToAppend)) {
            throw new Exception("Could not append a valid sequence to Kafka.");
        }
    }

    private boolean validateSequence(String topicUri,
                                     final int parallelism,
                                     PscDeserializationSchema<Tuple2<Integer, Integer>> deserSchema,
        ...
        Properties readConfiguration = new Properties();
        readConfiguration.putAll(standardPscConsumerConfiguration);
        readConfiguration.setProperty(PscConfiguration.PSC_CONSUMER_GROUP_ID, "flink-tests-validator");
            ...
            if (!killedLeaderBefore) {
                ...
                if (failer && numElementsTotal >= failCount) {
                    ...
                    for (KafkaServer server : pscTestEnvWithKafka.getBrokers()) {
                        ...
                        if (pscTestEnvWithKafka.getBrokerId(server) == shutdownBrokerId) {
                            toShutDown = server;
                            break;
                        }
                    }
    ...
    private abstract static class AbstractTestDeserializer
            implements PscDeserializationSchema<Tuple3<Integer, Integer, String>> {
        ...
        @Override
        public Tuple3<Integer, Integer, String> deserialize(PscConsumerMessage<byte[], byte[]> record) throws Exception {
            DataInputView in = new DataInputViewStreamWrapper(
                    new ByteArrayInputStream(record.getValue()));
            Tuple2<Integer, Integer> t2 = ts.deserialize(in);
            return new Tuple3<>(t2.f0, t2.f1,
                    record.getMessageId().getTopicUriPartition().getTopicUriAsString());
        }
    ...
    private static class TestDeserializer extends AbstractTestDeserializer
            implements PscSerializationSchema<Tuple3<Integer, Integer, String>> {
        ...
        @Override
        public PscProducerMessage<byte[], byte[]> serialize(Tuple3<Integer, Integer, String> element,
            ...
            return new PscProducerMessage<>(element.f2, serializedValue);
        }

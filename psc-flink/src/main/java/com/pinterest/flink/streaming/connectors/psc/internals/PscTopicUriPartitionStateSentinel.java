 ...
 *
 * <p>The values are all negative. Negative offsets are not used by backend pubsub (invalid), so we
 * pick a number that is probably (hopefully) not used by backend pubsub as a magic number for anything else.
 */
@Internal
public class PscTopicUriPartitionStateSentinel {
    ...
    /**
     * Magic number that defines the partition should start from its committed group offset.
     *

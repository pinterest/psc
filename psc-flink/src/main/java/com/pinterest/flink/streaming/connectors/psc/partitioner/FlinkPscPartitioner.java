...
package com.pinterest.flink.streaming.connectors.psc.partitioner;
...
/**
 * A {@link FlinkPscPartitioner} wraps logic on how to partition records
 * across partitions of multiple PSC topic URIs.
 */
@PublicEvolving
public abstract class FlinkPscPartitioner<T> implements Serializable {
     ...
     * the Flink PSC producer. This method should be overridden if necessary.
     *
     ...
     * @param targetTopicUri target topic for the record
     * @param partitions     found partitions for the target topic
     * @return the id of the target partition
     */
    public abstract int partition(T record, byte[] key, byte[] value, String targetTopicUri, int[] partitions);
}

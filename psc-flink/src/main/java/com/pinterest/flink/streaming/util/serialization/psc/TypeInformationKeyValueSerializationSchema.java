 ...
 */
@PublicEvolving
public class TypeInformationKeyValueSerializationSchema<K, V> implements PscDeserializationSchema<Tuple2<K, V>>, KeyedSerializationSchema<Tuple2<K, V>> {
    ...
    @Override
    public Tuple2<K, V> deserialize(PscConsumerMessage<byte[], byte[]> record) throws Exception {
        K key = null;
        V value = null;

        if (record.getKey() != null) {
            inputDeserializer.setBuffer(record.getKey());
            key = keySerializer.deserialize(inputDeserializer);
        }
        if (record.getValue() != null) {
            inputDeserializer.setBuffer(record.getValue());

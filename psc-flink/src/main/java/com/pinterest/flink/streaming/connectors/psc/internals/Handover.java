 ...
 *
 * <p>This class is used in the Flink PSC Consumer to hand over data and exceptions between
 * the thread that runs the PscConsumer class and the main thread.
 *
...
public final class Handover implements Closeable, EventHandler {
     ...
     */
    @Nonnull
    public PscConsumerPollMessageIterator<byte[], byte[]> pollNext() throws Exception {
        synchronized (lock) {
            while (next == null && error == null) {
                lock.wait();
            }

            PscConsumerPollMessageIterator<byte[], byte[]> n = next;
            if (n != null) {
                next = null;
                lock.notifyAll();
                return n;
                ...
                return PscConsumerPollMessageIterator.emptyIterator();
            }
        }
    }
     ...
     * @throws InterruptedException Thrown, if the thread is interrupted while blocking for the Handover to be empty.
     * @throws WakeupException      Thrown, if the {@link #wakeupProducer()} method is called while blocking for
     *                              the Handover to be empty.
     * @throws ClosedException      Thrown if the Handover was closed or concurrently being closed.
     */
    public void produce(final PscConsumerPollMessageIterator<byte[], byte[]> element)
     ...
     * calls that method.
     *
     * <p>After this method has been called, no call to either {@link #produce(PscConsumerPollMessageIterator)} )}
    ...
    /**
     * Closes the handover. Both the {@link #produce(PscConsumerPollMessageIterator)} method and the
     ...
     * the {@link #produce(PscConsumerPollMessageIterator)} method, it will exit the method throwing
     ...
     * {@link #produce(PscConsumerPollMessageIterator)} method, after the Handover was closed via
     * {@link #close()}.
     */
    ...
    /**
     * A special exception thrown bv the Handover in the {@link #produce(PscConsumerPollMessageIterator)}

...
/** A specific {@link PscSerializationSchema} for {@link PscDynamicSource}. */
class DynamicPscDeserializationSchema implements PscDeserializationSchema<RowData> {
    ...
    @Override
    public RowData deserialize(PscConsumerMessage<byte[], byte[]> message) throws Exception {
        throw new IllegalStateException("A collector is required for deserializing.");
    }

    @Override
    public void deserialize(PscConsumerMessage<byte[], byte[]> message, Collector<RowData> collector)
            throws Exception {
        ...
        // buffer key(s)
        if (keyDeserialization != null) {
            keyDeserialization.deserialize(message.getKey(), keyCollector);
        }
        ...
        if (message.getValue() == null && upsertMode) {
            // collect tombstone messages in upsert mode by hand
            outputCollector.collect(null);
        } else {
            valueDeserialization.deserialize(message.getValue(), outputCollector);
        }
    ...
    interface MetadataConverter extends Serializable {
        Object read(PscConsumerMessage<?, ?> message);
    }
    ...
    private static final class OutputProjectionCollector
        ...
        private transient PscConsumerMessage<?, ?> inputRecord;

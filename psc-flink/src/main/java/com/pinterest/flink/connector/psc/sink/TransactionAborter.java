...
class TransactionAborter implements Closeable {
    ...
    private final Consumer<FlinkPscInternalProducer<byte[], byte[]>> closeAction;
    @Nullable
    ...
    public TransactionAborter(
            int subtaskId,
            int parallelism,
            Function<String, FlinkPscInternalProducer<byte[], byte[]>> producerFactory,
            Consumer<FlinkPscInternalProducer<byte[], byte[]>> closeAction) {

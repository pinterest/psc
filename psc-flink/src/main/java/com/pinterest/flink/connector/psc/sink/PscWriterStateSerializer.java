...
class PscWriterStateSerializer implements SimpleVersionedSerializer<PscWriterState> {
    ...
    @Override
    public byte[] serialize(PscWriterState state) throws IOException {
    ...
    @Override
    public PscWriterState deserialize(int version, byte[] serialized) throws IOException {
        try (final ByteArrayInputStream bais = new ByteArrayInputStream(serialized);
                final DataInputStream in = new DataInputStream(bais)) {
            final String transactionalIdPrefx = in.readUTF();
            return new PscWriterState(transactionalIdPrefx);
        }
    }
}
